...
export function ClientSpaceLayout() {
  ...
  const activeItemId = resolveActiveClientTab(location.pathname, searchParams);
...
function resolveActiveClientTab(pathname: string, searchParams: URLSearchParams): ClientSpaceTab {
  if (pathname.includes('/etude/')) return searchParams.get('source') === 'ARCHIVES' ? 'ARCHIVES' : 'ETUDES';
  if (pathname.includes('/demande/')) return 'DEMANDES';
  const tab = searchParams.get('tab');
  return tab === 'ETUDES' || tab === 'ARCHIVES' ? tab : 'DEMANDES';
}
